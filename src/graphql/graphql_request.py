...
from src.graphql.ssot_enums import BooleanPropertyType, GroupByType, MetricType, Operator, SexType, StrokeType
...
class GraphQLQueryGenerator:
    ...
    def generate(request: GraphQLQueryRequest) -> str:
        ...
        filter_args.append(f'''
            timePeriod: [[
                {{startDate: "{request.time_period.start_date}"}},
                {{endDate: "{request.time_period.end_date}"}}
            ]]
        ''')
    ...
    def _generate_metric_field(metric: MetricRequest) -> str:
        ...
        # Build kpiGroup fields
        kpi_group_fields = [
            f"""
            kpi1: kpi({kpi_options}) {{
                {" ".join(kpi_fields)}
            }}
        """
        ]
