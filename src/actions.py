...
from src.langchain.planner_chain import generate_analysis_plan
        ...
        try:
            ...
            try:
                if hasattr(plan_obj, "model_dump_json"):
                    ...
                    plan_json = _json.dumps(plan_obj, indent=2, default=str)
            except Exception as ser_exc:
                plan_json = f"Serialization error: {ser_exc}"
            dispatcher.utter_message(text=plan_json)

            logger.info(f"Successfully processed visualization request for user: {user_id}")
