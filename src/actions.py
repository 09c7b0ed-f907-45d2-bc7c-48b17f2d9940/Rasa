...
from graphql_builder import Metric, MetricBuilder, build_query
from graphql_client import GraphQLClient
from vault_client import VaultClient

logger = logging.getLogger(__name__)

VaultAddress, VaultToken, GraphQLURL = require_env("VAULT_ADDRESS", "VAULT_TOKEN", "GRAPHQL_URL")

vault_client = VaultClient(VaultAddress, VaultToken)
graphql_client = GraphQLClient(GraphQLURL)
...
class ActionTestAPIs(Action):
    ...
    async def run(
        ...
        try:
            ...
            if token:
                dispatcher.utter_message(text="Successfully retrieved token from Vault.")
                queryMinimal = graphql_client.query(build_query([MetricBuilder(Metric.Dtn).with_stats()]), token)
                if queryMinimal:
                    dispatcher.utter_message(text="Successfully queried GraphQL API.")

                else:
                    dispatcher.utter_message(text="Failed to query GraphQL API.")
                    logger.error("Query returned no data.")
            else:
                dispatcher.utter_message(text="Unable to retrieve token from Vault.")
...
class ActionGetDTN(Action):
    ...
    async def run(
        ...
        if token:
            queryMinimal = graphql_client.query(build_query([MetricBuilder(Metric.Dtn).with_stats()]), token)
            dispatcher.utter_message(text=json.dumps(queryMinimal, indent=1))
        else:
            dispatcher.utter_message(text="Unable to retrieve token from Vault.")
...
#                dispatcher.utter_message(text="Sorry, I don't understand. Can you try rephrasing?")
...
#        logger.info("Toggling National Value")
...
#            text="We are {} the national value.".format("showing" if new_value else "hiding")
#        )
...
#                feat: round(weight, 2) for feat, weight in feature_weights["Shap Values"].items()
#            }
#
#            selected_value = tracker.get_slot("selected_value")
#            dispatcher.utter_message(
...
#            )
#            dispatcher.utter_message("The most important factors are:")
